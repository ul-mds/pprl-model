...
class EntityMaskRequest(BaseMaskRequest):
    ...
    def validate_attribute_type(self) -> Self:
        if self.config.filter.type == FilterType.clk:
            ...
            raise ValueError(
                f"`{self.config.filter.type.value}` filters require static attribute configurations, but "
                f"weighted ones were found"
            )
        else:
            if len(self.attributes) == 0:
                raise ValueError(
                    f"`{self.config.filter.type.value}` filters require weighted attribute "
            ...
            raise ValueError(
                f"`{self.config.filter.type.value}` filters require weighted attribute configurations, "
